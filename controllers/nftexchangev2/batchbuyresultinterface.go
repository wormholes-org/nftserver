...
func (nft *NftExchangeControllerV2) BatchBuyResultInterface() {
	fmt.Println("BatchBuyResultInterface()>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", time.Now())
	var httpResponseData controllers.HttpResponseData
	nd := new(models.NftDb)
	nd, err := models.NewNftDb(models.Sqldsndb)
	if err != nil {
		fmt.Printf("BatchBuyResultInterface() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err == nil {
		...
		inputDataErr := nft.verifyInputData_BatchBuyResultInterface(data)
		if inputDataErr != nil {
...
func (nft *NftExchangeControllerV2) verifyInputData_BatchBuyResultInterface(data map[string]string) error {
