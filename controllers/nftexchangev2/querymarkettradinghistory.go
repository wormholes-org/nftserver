...
func (nft *NftExchangeControllerV2) QueryMarketTradingHistory() {
	...
	if err != nil {
		fmt.Printf("QueryMarketTradingHistory() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		if inputDataErr != nil {
			...
			userTradings, totalCount, err := nd.QueryMarketTradingHistory(data.Nfttype, data.Filter, data.Sort, data.StartIndex, data.Count)
			if err != nil {
