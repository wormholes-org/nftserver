...
func (nft *NftExchangeControllerV2) QuerySnftOtherChip() {
	fmt.Println("QuerySnftOtherChip()>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", time.Now())
	var httpResponseData controllers.HttpResponseData
	nd, err := models.NewNftDb(models.Sqldsndb)
	if err != nil {
		fmt.Printf("QuerySnftOtherChip() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		if inputDataErr != nil {
			...
			snfts, count, err := nd.QuerySnftOtherChip(data["user_addr"], data["nft_contract_addr"], data["nft_token_id"])
			if err != nil {
				if err == gorm.ErrRecordNotFound || err == models.ErrNftNotExist {
					httpResponseData.Code = "200"
				} else {
