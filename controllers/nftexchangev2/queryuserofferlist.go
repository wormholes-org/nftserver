...
//Get a list of user bids
func (nft *NftExchangeControllerV2) QueryUserOfferList() {
	fmt.Println("QueryUserOfferList()>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", time.Now())
	var httpResponseData controllers.HttpResponseData
	nd, err := models.NewNftDb(models.Sqldsndb)
	if err != nil {
		fmt.Printf("QueryUserOfferList() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		inputDataErr := nft.verifyInputData_QueryUserOfferList(data)
		if inputDataErr != nil {
			httpResponseData.Code = "500"
			httpResponseData.Msg = inputDataErr.Error()
			httpResponseData.Data = []interface{}{}
		} else {
			userOffers, totalCount, err := nd.QueryUserOfferList(data["user_addr"], data["start_index"],
				data["count"])
			if err != nil {
...
func (nft *NftExchangeControllerV2) verifyInputData_QueryUserOfferList(data map[string]string) error {
