...
func (nft *NftExchangeControllerV2) QueryUserTradingHistory() {
	...
	if err != nil {
		fmt.Printf("QueryUserTradingHistory() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		inputDataErr := nft.verifyInputData_QueryUserTradingHistory(data)
		if inputDataErr != nil {
			httpResponseData.Code = "500"
			httpResponseData.Msg = inputDataErr.Error()
			httpResponseData.Data = []interface{}{}
		} else {
			userTradings, totalCount, err := nd.QueryUserTradingHistory(data["user_addr"], data["start_index"],
				data["count"])
			if err != nil {
...
func (nft *NftExchangeControllerV2) verifyInputData_QueryUserTradingHistory(data map[string]string) error {
