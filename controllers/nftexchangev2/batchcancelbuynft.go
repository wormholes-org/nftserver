...
func (nft *NftExchangeControllerV2) BatchCancelBuyNft() {
	fmt.Println("BatchCancelBuyNft()>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", time.Now())
	var httpResponseData controllers.HttpResponseData
	nd, err := models.NewNftDb(models.Sqldsndb)
	if err != nil {
		fmt.Printf("BatchCancelBuyNft() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		inputDataErr := nft.verifyInputData_BatchCancelBuyNft(data, token)
		if inputDataErr != nil {
			httpResponseData.Code = "500"
			...
			if err != nil {
				...
				if err != nil {
					...
					err := nd.BatchCancelBuy(data["user_addr"], data["offer_list"])
					if err != nil {
						httpResponseData.Code = "500"
						httpResponseData.Msg = err.Error()
						httpResponseData.Data = []interface{}{}
...
func (nft *NftExchangeControllerV2) verifyInputData_BatchCancelBuyNft(data map[string]string, token string) error {
