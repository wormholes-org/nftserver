...
func (nft *NftExchangeControllerV2) QueryOwnerSnftChipAmount() {
	...
	if err != nil {
		fmt.Printf("QueryOwnerSnftChipAmount() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err != nil {
		...
		inputDataErr := nft.verifyInputData_QueryOwnerSnftChipAmount(data)
		if inputDataErr != nil {
			httpResponseData.Code = "500"
			httpResponseData.Msg = inputDataErr.Error()
			httpResponseData.Data = []interface{}{}
		} else {
			count, err := nd.QueryOwnerSnftChipAmount(data["owner_addr"], "")
			if err != nil {
...
func (nft *NftExchangeControllerV2) verifyInputData_QueryOwnerSnftChipAmount(data map[string]string) error {
	...
	if data["owner_addr"] != "" {
		...
		if !match {
			fmt.Println("verifyInputData_QueryOwnerSnftChipAmount() owner_addr error", data["owner_addr"])
			return ERRINPUTINVALID
		}
	}
	/*if data["name"] != "" {
