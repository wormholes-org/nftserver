...
func (nft *NftExchangeControllerV2) QueryNftList() {
	...
	if err != nil {
		fmt.Printf("QueryNftList() connect database err = %s\n", err)
		return
	}
	defer nd.Close()
	...
	if err == nil {
		...
		if inputDataErr != nil {
			...
			nfts, totalCount, err := nd.QueryNftByFilterNftSnft(data.Filter, data.Sort, data.Nfttype, data.StartIndex, data.Count)
			if err == nil {
