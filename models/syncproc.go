...
func GetDbBlockNumber(sqldsn string) (uint64, error) {
	...
	defer nd.Close()
	var params SysParams
	dbErr := nd.db.Select("scannumber").Last(&params)
	if dbErr.Error != nil {
...
func GetDbSnftBlockNumber(sqldsn string) (uint64, error) {
	...
	defer nd.Close()
	var params SysParams
	dbErr := nd.db.Select("scansnftnumber").Last(&params)
	if dbErr.Error != nil {
		fmt.Println("GetDbBlockNumber() opendb err=", dbErr.Error)
		return 0, dbErr.Error
	}
...
func TransProc(tx *gorm.DB, from, to, price, contractAddr, tokenId, txhash, ts string) error {
	...
	if err.Error != nil {
		...
		return tx.Transaction(func(tx *gorm.DB) error {
			...
			err = tx.Where("createaddr = ? AND  name=?",
...
func AmountValid(Price uint64, addr string) (bool, string, error) {
	...
	auth, err := contracts.AllowanceOfWeth(addr)
	if err != nil {
		return false, "", err
	}
	//balance, err := ethhelper.BalanceOfWeth(addr)
	balance, err := contracts.BalanceOfWeth(addr)
	if err != nil {
		return false, "", err
	}
...
func WormsAmountValid(Price uint64, addr string) (bool, string, error) {
	temp, err := contracts.WormsBalance(addr)
	if err != nil {
		return false, err.Error(), err
	}
	fmt.Println("WormsAmountValid() balance =", temp)
	if len(temp) < 9 {
		return false, ErrBalanceLess.Error(), nil
	} else {
		temp = temp[:len(temp)-9]
...
/*func OwnAndAprove(owner, contract, tokenId string) (bool, error) {
	...
	if ct == 1 {
		...
		if err != nil {
			fmt.Println("OwnAndAprove() NFT721 err=", err)
			return false, err
		}
		...
		isOwner, err := ethhelper.IsOwnerOfNFT1155(owner, contract, tokenId)
		if err != nil {
			fmt.Println("OwnAndAprove() NFT1155 err=", err)
			return false, err
		}
		approve, err := ethhelper.IsApprovedNFT1155(owner, contract)
		if err != nil {
			fmt.Println("OwnAndAprove() NFT1155 err=", err)
			return false, err
		}
