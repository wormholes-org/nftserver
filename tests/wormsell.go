...
type WormResponseSell struct {
	Code 	string 		`json:"code"`
	Msg 	string 		`json:"msg"`
	Totalcount int		`json:"total_count"`
}

func WormSell(userAddr, sellType, contract, tokenId, price, tradeSign string, token string, workKey, userKey *ecdsa.PrivateKey) error {
	url := SrcUrl + "sell"
...
func WormMakeOffer(userAddr, contract, tokenId, price, tradeSign string, token string, userKey, workKey *ecdsa.PrivateKey) error {
	url := SrcUrl + "buy"
	datam := make(map[string]string)
...
func WormCancelMakeOffer(userAddr string, contract string, tokenId string, token string, workKey *ecdsa.PrivateKey) error {
...
func WormCancelSell(userAddr string, contract string, tokenId string, token string, workKey *ecdsa.PrivateKey) error {
...
func WormBuyMint(contract, from, to, tokenId, royalty, price string, mintSig, tradeSig []byte, fromKey *ecdsa.PrivateKey) (*types.Transaction, error) {
...
func WormBuy(contract, from, to, tokenId, price string, tradeSig []byte, fromKey *ecdsa.PrivateKey) (*types.Transaction, error) {
	client, err := ethclient.Dial(EthNode)
	//client, err := ethclient.Dial("http://127.0.0.1:8545")
	if err != nil {
		fmt.Println("Buy() err=", err)
...
func WormBuyBidding(contract, from, to, tokenId, price string, tradeSig []byte, fromKey *ecdsa.PrivateKey) (*types.Transaction, error) {
	client, err := ethclient.Dial(EthNode)
	//client, err := ethclient.Dial("http://127.0.0.1:8545")
	if err != nil {
		fmt.Println("Buy() err=", err)
...
/*
data["user_addr"], data["buyer_Addr"], data["nft_contract_addr"],
						data["nft_token_id"], price, data["buyer_sig"]
*/
func WormBuying(userAddr, buyer_Addr, buyer_sig, contract, tokenId, price string, token string, userKey, workKey *ecdsa.PrivateKey) error {
	url := SrcUrl + "buying"

	datam := make(map[string]string)
	datam["user_addr"] = userAddr
	datam["buyer_Addr"] = buyer_Addr
	datam["nft_contract_addr"] = contract
	datam["nft_token_id"] = tokenId
	datam["buyer_sig"] = buyer_sig
	datam["price"] = price
